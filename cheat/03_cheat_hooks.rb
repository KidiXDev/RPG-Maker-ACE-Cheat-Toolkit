...
# that defers to the original behaviour unless the matching AsCheater flag is on.
#
...
class Game_Battler
  if method_defined?(:execute_damage)
    alias asc_god_execute_damage execute_damage
    def execute_damage(user)
      if AsCheater.god_mode && actor?
        @result.hp_damage = 0
        @result.mp_damage = 0
      end
      asc_god_execute_damage(user)
    end
  end

  if method_defined?(:regenerate_hp)
    alias asc_god_regenerate_hp regenerate_hp
    def regenerate_hp
      return if AsCheater.god_mode && actor? # ignore slip (poison) damage
      asc_god_regenerate_hp
    end
  end
end
...
class Game_BattlerBase
  if method_defined?(:die)
    alias asc_god_die die
    def die
      return if AsCheater.god_mode && respond_to?(:actor?) && actor?
      asc_god_die
    end
  end
end
...
class Game_Player
  alias asc_clip_passable? passable?
  def passable?(x, y, d)
    return true if AsCheater.no_clip
    asc_clip_passable?(x, y, d)
  end
end
